...
- [x] 29. **TEST-3 (final part): automated E2E in CI.** ✅ 2026-07-22: added Playwright with a full-stack smoke suite (`frontend/e2e/onboarding.spec.ts`) that drives the real backend + Postgres + Redis through the Vite proxy — the onboarding journey creates an actual account, and the test asserts `anchor_user_id` is persisted (only true after a successful `/onboarding` round-trip), so a green run proves every layer is wired (the integration gap that let SEC-1/SEC-2 through). New `e2e` npm script + `playwright.config.ts` (Playwright owns the frontend dev server; the caller starts the backend). New CI `e2e` job: Postgres+Redis services, migrate, background uvicorn, `playwright install --with-deps chromium`, run the suite, upload the HTML report artifact. vitest scoped to `src/` so it no longer collides with the `.spec.ts` E2E files. Verified locally against the real stack: 2/2 pass; lint/tsc/vitest(18)/build all still green; gitleaks clean.
- [x] 28. **AI-1 (final part): fallback-rate alerting wired in.** ✅ 2026-07-21: the fallback counters (`GET /v1/metrics/ai`, from step 7) were observable but nothing alerted. `app/ai/metrics.py` now keeps a rolling window of recent AI dispatches and, when the recent fallback rate crosses `AI_FALLBACK_ALERT_THRESHOLD` (default 50% over the last 20 calls), emits a structured ERROR log **and** a `sentry_sdk.capture_message` — cooldown-limited to once per `AI_FALLBACK_ALERT_COOLDOWN_SECONDS` (default 5 min) so an outage alerts once, not per request. All alert-path failures are swallowed so alerting can never break an AI call. Config knobs added to `Settings`; new `capture_message` helper in `observability.py`. Log/Sentry-based alerting is the realistic hook for this stack (no Prometheus); the raw counters stay scrapeable. 2 new tests (fires-once-then-cooldown; no-alert-below-threshold); 289 pass / 0 ruff / 0 mypy.
- [x] 27. **DATA-3 (final part): Privacy Policy + ToS drafts.** ✅ 2026-07-21: the last open piece of DATA-3 — `docs/legal/PRIVACY_POLICY.md` and `docs/legal/TERMS_OF_SERVICE.md`, drafted from the app's **actual** data practices (argon2id passwords; AES-256-GCM encryption of coaching content / mood notes / RSD triggers / AI feedback; pseudonymous `rsd_logs`; Anthropic as the AI sub-processor with `share_ai_context` consent gating default-OFF; cascade + scheduled account deletion; ~7-day backup ageout; crisis-detection disclaimer). Both are clearly marked **DRAFT — pending legal review** with `[[…]]` operator/legal placeholders (entity, jurisdiction, sub-processor terms, liability). Not published anywhere — repo docs only. DEPLOYMENT.md retention note and docs index updated to point at them. _(Backups/retention working-policy from DATA-3 were already done in step 14; this closes the documents gap. Formal counsel review + publication still required before public launch — inherently a human/legal step.)_
- [x] 26. **FE-5: PWA / service-worker review.** ✅ 2026-07-21: found and fixed a real bug — `sendSubscriptionToServer` POSTed to a non-existent `/api/v1/notifications/subscribe` path (the app's base is `/v1`); it now goes through the shared `api` client, which also carries the session cookie so the auth-gated route actually authenticates. Added an **offline navigation fallback**: a workbox `setCatchHandler` returns the precached `index.html` shell when a navigation can't be served from network or cache (first-visit-offline / uncached deep link), so the SPA still boots and shows its own offline banner instead of the browser error page. Made `autoUpdate` actually work — the SW now `skipWaiting()` + `clientsClaim()` so a fresh bundle activates and takes over open tabs instead of sitting "waiting" (the stale-bundle risk the audit flagged). VAPID stays optional and degrades gracefully (empty key → no subscription, no error); documented the keys in `.env.production.example` (they were only in `.env.example`) so an operator knows push is opt-in. Verified: clean tsc + eslint, `npm run build` OK, built `sw.js` contains the catch handler + skipWaiting, 18 tests pass.
- [x] 25. **SEC-8: gitleaks is the primary secret scanner.** ✅ 2026-07-21: the old `check-secrets.sh` only matched `sk-ant-`/`AIza`/PEM headers — it missed JWTs, DB URLs with passwords, cloud tokens, etc. Added gitleaks (audited default ruleset) as the primary CI scanner, running `gitleaks git .` over full history on every push/PR; the regex script stays as a fast backstop. `.gitleaks.toml` extends the default rules with a narrow, reviewed allowlist (non-source paths + two generic-api-key false positives: `anchor_*_v1` localStorage keys and an `xp_awarded=` assignment). Verified locally: working tree, git history, and the module-level app all scan clean, while a freshly-generated RSA key is still flagged (allowlist doesn't blunt detection). The workflow installs the pinned gitleaks binary directly to avoid the org-licensed gitleaks-action. _(`creds.md` was already removed before the first commit in step 1.)_
- [x] 24. **OBS-3: split liveness / readiness probes.** ✅ 2026-07-21: `/v1/health` previously returned 200 even when Redis was down, so orchestration had to parse the body to notice trouble. Added `/v1/health/live` (liveness — no dependency checks, always 200, so a Redis blip never restarts the container) and `/v1/health/ready` (readiness — **503** when the DB, a hard dependency, is unreachable so the instance leaves rotation; Redis is soft and only downgrades the reported status, staying 200). `/v1/health` kept as the rich human/dashboard view. docker-compose backend healthcheck now hits `/v1/health/ready` (curl `-f` fails on the 503). 4 new tests; 287 pass / 0 ruff / 0 mypy.
- [x] 23. **BE-9: API docs gated out of production.** ✅ 2026-07-21: Swagger (`/v1/docs`), ReDoc (`/v1/redoc`), and the raw OpenAPI schema (`/v1/openapi.json`) — which together enumerate every route and payload shape — now serve only when `app_env != "production"` (`docs_url`/`redoc_url`/`openapi_url` set to `None` in prod). Dev/staging keep them for convenience. The existing CSP docs-exemption is unaffected (it just never matches in prod). 5 new tests; 283 pass / 0 ruff / 0 mypy.
